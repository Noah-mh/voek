...
const ViewReceived = ({ receivedOrders, getAll }: Props) => {
    ...
    axiosPrivateCustomer
      ...
      .then((response) => {
        ...
        const imageUploadPromises = image_urls.map((image_url) => {
          return axiosPrivateCustomer.post(
            "/addReviewImages",
            JSON.stringify({ review_id, image_url }),
            {
              headers: { "Content-Type": "application/json" },
              withCredentials: true,
            }
  ...
  useEffect(() => {
    axiosPrivateCustomer
      .get(`/customer/ratedOrNot/${customer_id}`)
      .then((response) => {
        const newRatings: { [key: number]: boolean } = {};

        response.data.rated.forEach(
          (item: { rated: any; orders_product_id: number }) => {
      ...
      {orderedReceivedOrders?.map((ordersArray: any) => (
        <div key={uuidv4()} className="mb-8  shadow-md rounded p-4">
          ...
          <h1 className="text-md font-bold opacity-40">
            Receieved Date: {getLocalDate(ordersArray[0][0].shipment_delivered)}{" "}
            {getLocalTime(ordersArray[0][0].shipment_delivered)}
          </h1>
        </div>
      ))}
    </div>
